...
# 1PL / 2PL / 3PL, ON A PROOF-OF-CONCEPT SET OF IRW TABLES
#
# This is a thin IRW harness around the single-dataset engine in
# IRW_personfit_Sept09092026.R (`analyze_irw_personfit()`), which
...
#      1PL, 2PL and 3PL, selects a working model by BIC,
#      computes five person-fit indices (lz*, U*, W*, ECI_Z2*,
#      ECI_Z4*) under every model that calibrates, and returns
#      model comparison, flagging, distribution, ability-
#      conditional, cross-model stability and parametric
#      bootstrap Type I error tables.
...
source("IRW_personfit_Sept09092026.R")
...
# Proof-of-concept selection: four small/medium tables drawn from the
# 18 that calibrated successfully in the July 3PL-only pilot. All are
# complete (density 1.00) and range from 316 to 757 persons, which
# keeps the 1PL/2PL/3PL triple plus a 50-replicate parametric
# bootstrap tractable while the pipeline is still being iterated on.
poc_datasets <- c(
  "verbagg",       # 316 persons x 24 items
  "janssen2_tam",  # 346 x 20
  "frac20",        # 536 x 20
  "cdm_timss03"    # 757 x 23
)
...
settings <- list(
  ...
  engine_script = "IRW_personfit_Sept09092026.R"
)
